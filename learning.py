...
from custom_logger.CustomLogging import configure_logger
...
configure_logger('custom_logger/logging.yaml')
...
for oiu in range(1,2):
    logger.info(f'Object {oiu}')

    # Load model information
    model_info = load_model_info(dataset_path, oiu)
    ...
    # po_image = PODestarisation(model_info,amount_of_instances = 1)(cnn_po_star, dash_image, isvalid, inputs['roation_matrix'])
    # po_uv, po_cam = loss.Po_to_Img()(po_image, inputs['camera_matrix'], inputs['roation_matrix'], inputs['translation'])
    ...
    #     for data in DataLoader.CustomDataset(train_data, xyDim, times=2, group_size=5, random=True).batch(80).prefetch(20).map(totuplething):
    #         inputs, targets = data
    ...
    #     train_model.train()
    #     for data in DataLoader.CustomDataset(train_data, xyDim, times=2, group_size=5, random=True).batch(40).prefetch(20).map(totuplething):
    #         inputs, targets = data
    #         inputs = [input.to(device) for input in inputs]
    #         targets = [target.to(device) for target in targets]
